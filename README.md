...
一个 Windows 桌面工具，定时自动清理用户指定文件夹中的冗余文件。适用场景：下载目录、AI缓存目录、日志目录等容易膨胀的文件夹，设置一次后常驻后台自动维护。
...
| **开机自启** | 勾选后自动写入注册表自启项（免管理员权限），开机常驻后台自动维护 |
| **单实例** | 只允许运行一个实例，重复启动自动被拒绝并拉出已有窗口 |
...
- **v1.22** 开机自启真正生效（写注册表 Run 键，启动/保存设置时自动同步）
- **v1.21** 单实例限制（只允许运行一个实例）
- **v1.20** 项目目录整理
